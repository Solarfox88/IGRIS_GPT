...
class AgentReasoningLoop:
    ...
    def run(
        ...
        for step_num in range(1, self.max_steps + 1):
            ...
            # automatically so the work is not lost to cleanup. The model can still
            # add server.py wiring in subsequent steps. (#1069)
    ...
    def _execute_tool_runtime(self, action) -> Dict[str, Any]:
        ...
        try:
            ...
            if action.action_type == "git_status":
                ...
                # This is the primary path for the reasoning loop to persist
                # its work. Without this handler the model receives
                # "Tool runtime action not yet integrated" and never commits —
                # causing chronic no_diff_repair failures.
                files = action.parameters.get("files") or []
                message = action.parameters.get("message") or action.parameters.get("commit_message") or "feat: reasoning loop implementation"
                ...
                # Stage individual files without committing.
                files = action.parameters.get("files") or action.parameters.get("paths") or []
                if not files:
                    path = action.parameters.get("path") or action.parameters.get("file") or "."
                    files = [path]
                results = []
                for f in files:
