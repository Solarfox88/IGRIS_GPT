...
class SupervisorBackend(Protocol):
    ...
    def run_tests(self, targets: Optional[List[str]] = None, timeout: int = 120, hard_cap: int = 3600, exclude_slow: bool = False) -> CommandResult: ...
    ...
    def call_api_helper(self, packet: Dict[str, Any], model: str, max_tokens: int, timeout: int = 45, mode: str = "") -> CommandResult: ...
    def api_helper_is_configured(self) -> bool: ...
    def git_apply_patch(self, patch_path: str, timeout: int = 30) -> CommandResult: ...
...
class LocalSupervisorBackend:
    ...
    def git_apply_patch(self, patch_path: str, timeout: int = 30) -> CommandResult:
        return self._run(["git", "apply", "--index", patch_path], timeout=timeout)
