...
def test_fallback_response_is_contextual():
    assert "status" in _build_fallback_response("show me the status").lower() or \
           "/api/" in _build_fallback_response("show me the status")
    assert "task" in _build_fallback_response("create a task").lower() or \
           "/api/tasks" in _build_fallback_response("create a task")
    assert "test" in _build_fallback_response("run tests").lower() or \
           "runner" in _build_fallback_response("run tests").lower()
