...
def run_replay(project_root: str = ".") -> Dict[str, object]:
    out_dir = Path(project_root) / "reports" / "mission_brain" / "hardening" / "824"
        ...
        results.append(
            {
                ...
                "declared_status_after_824": mission.status,
                "quality_passed": quality["passed"],
                "satisfaction_passed": satisfaction["passed"],
                "insufficient_evidence_detected": any(
                    "insufficient action evidence depth" in gap.lower() for gap in quality.get("gaps", [])
                ),
    ...
    summary = {
        "suite": "mission_brain_hardening_824_replay",
        "cases": results,
        "all_cases_diagnostic_partial_or_failed": all(
            (not item["quality_passed"]) and item["insufficient_evidence_detected"] for item in results
        ),
    }
    (out_dir / "hardening_824_replay.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    return summary
