...
| 2026-08-14 | #1316 Phase 1 / PR TBD | Devin | fix(#1316): Phase 1 — pyright type checking; added `[tool.pyright]` to pyproject.toml (basic mode, Python 3.12, include igris/, exclude .local/); added `type-check` job to CI workflow (pyright --outputjson); added pyright to dev dependencies | `tests/test_pyright_config.py` (7/7) | (pending merge) |
